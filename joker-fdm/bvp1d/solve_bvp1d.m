...
#   p -- parameters of the functions f
#   g -- right-hand sides
...
function sol = solve_bvp1d (grid_info, data, guess, tol, max_iter = Inf, silent = true)
...
function [coeff, rhs] = get_nonlinear_term (grid_info, data, guess, i, j, n)
  ...
  coeff(ind) = arrayfun(@(k) data.df{i, j, k}( guess(k, gind), data.p(i, k, gind) ), 1 : N);
  rhs = data.g(i, gind) + sum(arrayfun( @(k) ( ...
    data.df{i, j, k}( guess(k, gind), data.p(i, k, gind) ) * guess(k, gind) ...
    - data.f{i, j, k}( guess(k, gind), data.p(i, k, gind) )  ), 1 : N));
endfunction
