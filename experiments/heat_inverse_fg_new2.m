...
function ret = f1 (x)
  ...
  ret = f_cos(5, 10, x); #- маленький источник!
...
function ret = f2 (x)
  ...
  ret = f_cos(35, 40, x); #- маленький источник!
...
#  ret = f_cos(17, 18, x); #- маленький измеритель!
#endfunction
...
#  ret = f2(x);
  ...
  #ret = f_cos(27, 28, x); #- маленький измеритель!
...
#         @(x, p) -kappa_a*5*abs(x)^4, @(x, p) kappa_a};
  ...
  for i = 1:num_sources
    ...
    for k = 1:length(xgrid)
      if (meas(i) >= xgrid(k) && meas(i) < xgrid(k + 1))
        kk = k;
        break
      endif
    endfor
    r_vals(i) = theta(kk + 1) * (meas(i) - xgrid(kk)) / (xgrid(kk + 1) - xgrid(kk)) + ...
      theta(kk) * (xgrid(kk + 1) - meas(i)) / (xgrid(kk + 1) - xgrid(kk));
  endfor
endfunction
...
function [A, rhs] = calc_linear_system ()
  ...
  for eq_ind = 1 : num_sources
    ...
    for j = 1:num_sources
      ...
      kk = 1;
      for k = 1:length(xgrid)
        if (meas(j) >= xgrid(k) && meas(j) < xgrid(k + 1))
          kk = k;
          break
        endif
      endfor
      s_vals(j) = eta(kk + 1) * (meas(j) - xgrid(kk)) / (xgrid(kk + 1) - xgrid(kk)) + ...
        eta(kk) * (xgrid(kk + 1) - meas(j)) / (xgrid(kk + 1) - xgrid(kk));
...
function jac = calc_jacobian (theta)
  ...
  for i = 1:num_sources
    ...
    for j = 1:num_sources
      ...
      jac(j, i) = u_i(kk + 1) * (meas(j) - xgrid(kk)) / (xgrid(kk + 1) - xgrid(kk)) + ...
        u_i(kk) * (xgrid(kk + 1) - meas(j)) / (xgrid(kk + 1) - xgrid(kk));
...
contour(s1grid, s2grid, func_vals1', 0:0.01:15, 'k', 'ShowText', 'on')
hold on
