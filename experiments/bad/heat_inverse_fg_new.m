...
function ret = h1 (x)
#  ret = f_cos(25, 30, x);  #- вернуть этот код
...
function ret = h2 (x)
  #ret = f_cos(30, 35, x); #- вернуть этот код
...
atan2(B(1, 2), B(1, 1)) * 180 / pi
atan2(B(2, 2), B(2, 1)) * 180 / pi


# оценка якобиана
q = [0; 0];
[r_vals, theta] = calc_heat();
M_upper = calc_jacobian(theta)
#{
xgrid = linspace(0, L, K + 1);
figure
plot(xgrid, theta);
...
for q1_ind = 1:qn
  for q2_ind = 1:qn
    ...
    #Mat = calc_jacobian_naive(r_vals);
    angle1(q1_ind, q2_ind) = atan2(Mat(1, 2), Mat(1, 1)) / pi * 180;
    angle2(q1_ind, q2_ind) = atan2(Mat(2, 2), Mat(2, 1)) / pi * 180;
    delta_s = 0.001;
    delta_q = B^-1 * [0; delta_s];
    q1_new = q1_val + delta_q(1);
    q2_new = q2_val + delta_q(2);
    q(1) = q1_new;
    q(2) = q2_new;
    #[r_vals_new, theta_new] = calc_heat();
    #F1_new = r_vals_new(1);
    #R1(q1_ind, q2_ind) = (F1_new - r_vals(1)) / delta_s;
  endfor
endfor

#{
figure
#contour(q1grid, q2grid, angle1', 0:0.005:5, 'k', 'ShowText', 'on')
contour(q1grid, q2grid, angle1', 0:0.2:90, 'k', 'ShowText', 'on')
hold on
#contour(q1grid, q2grid, angle2', 0:0.005:5, 'k', '--', 'ShowText', 'on')
contour(q1grid, q2grid, angle2', 0:0.2:90, 'k', '--', 'ShowText', 'on')
#}

#contour(q1grid, q2grid, R1', 'k', 'ShowText', 'on')
